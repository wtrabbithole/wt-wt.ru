...
class ::gui_handlers.helpWndModalHandler extends ::gui_handlers.BaseGuiHandlerWT
{
  ...
  function initScreen()
  {
    ...
    local params = {
      showControlsAir  = (isContentControls || pageUnitType == ::g_unit_type.AIRCRAFT)
      showControlsTank = (isContentControls || pageUnitType == ::g_unit_type.TANK) && hasFeatureTanks
      showControlsShip = (isContentControls || pageUnitType == ::g_unit_type.SHIP) && hasFeatureShips
  ...
  function getPreferableTabId(params)
  {
    if (contentSet == HELP_CONTENT_SET.MISSION || contentSet == HELP_CONTENT_SET.CONTROLS)
    {
      ...
      if (::check_joystick_thustmaster_hotas(false) && pageUnitType == ::g_unit_type.AIRCRAFT)
      ...
      if (!isAdvanced && pageUnitType == ::g_unit_type.AIRCRAFT)
        return tabsCfg[help_tab_types.IMAGE_AIRCRAFT]
      if (!isAdvanced && pageUnitType == ::g_unit_type.TANK)
        return tabsCfg[help_tab_types.IMAGE_TANK]
      if (!isAdvanced && pageUnitType == ::g_unit_type.SHIP)
        return tabsCfg[help_tab_types.IMAGE_SHIP]
      if (params.hasController && pageUnitType == ::g_unit_type.AIRCRAFT)
        return tabsCfg[help_tab_types.CONTROLLER_AIR]
      if (params.hasController && pageUnitType == ::g_unit_type.TANK)
        return tabsCfg[help_tab_types.CONTROLLER_TANK]
      if (params.hasController && pageUnitType == ::g_unit_type.SHIP)
        return tabsCfg[help_tab_types.CONTROLLER_SHIP]
      if (params.hasKeyboard && pageUnitType == ::g_unit_type.AIRCRAFT)
        return tabsCfg[help_tab_types.KEYBOARD_AIR]
      if (params.hasKeyboard && pageUnitType == ::g_unit_type.TANK)
        return tabsCfg[help_tab_types.KEYBOARD_TANK]
      if (params.hasKeyboard && pageUnitType == ::g_unit_type.SHIP)
  ...
  function fillAllTexts()
  {
    ...
    local shortcutsList  = (pageUnitType == ::g_unit_type.AIRCRAFT) ? ::controlsHelp_shortcuts :
                           (pageUnitType == ::g_unit_type.TANK) ? ::controlsHelp_shortcuts_ground :
                           (pageUnitType == ::g_unit_type.SHIP) ? ::controlsHelp_shortcuts_naval : []
  ...
  function fillJoystickTexts()
  {
    ...
    for (local i=0; i<::shortcutsList.len(); i++)
    {
      ...
      if (type == CONTROL_TYPE.HEADER)
        ignoring = "unitType" in item && item.unitType != pageUnitType
      if (ignoring)
        continue
