...
class ::gui_handlers.WeaponWarningHandler extends ::gui_handlers.BaseGuiHandlerWT
{
  function initScreen()
  {
    scene.findObject("msgText").setValue(message)
    scene.findObject("listText").setValue(list)
    scene.findObject("skip_this").show(ableToStartAndSkip)
    scene.findObject("btn_select").show(ableToStartAndSkip)
    if (startBtnText!="")
  ...
  function onSkipWeaponWarning(obj)
  {
    if (!obj)
      return

    ::set_gui_option(skipOption, obj.getValue())
  ...
  wndType = handlerType.MODAL
  sceneBlkName = "gui/weaponry/weaponWarning.blk"
