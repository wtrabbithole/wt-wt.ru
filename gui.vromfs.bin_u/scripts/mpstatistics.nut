...
function build_mp_table(table, markup, hdr, max_rows)
{
  ...
  for (local i = 0; i < numRows; i++)
  {
    ...
    for (local j = 0; j < hdr.len(); ++j)
    {
      ...
      else if (::isInArray(hdr[j], [ "damageZone", "raceFinishTime", "raceLastCheckpoint", "raceLastCheckpointTime", "raceBestLapTime" ]))
      {
        local txt = isEmpty ? "" : ::g_mplayer_param_type.getTypeById(hdr[j]).printFunc(item, hdr[j], table[i])
      ...
      else
      {
        ...
        tdData += ::format("%s {" +
          ...
          "pare-text:t='yes';" +
          "%s}",
          tdProp+imageBg+textType, "txt_"+hdr[j], text, textParams+((imageBg=="")?"":"}")
        )
      }
...
function set_mp_table(obj_tbl, table, params)
{
  ...
  for (local i = 0; i < numRows; i++)
  {
    ...
    for (local idx = 0; idx < totalCells; idx++)
    {
      ...
      else if (::isInArray(hdr, [ "damageZone", "raceFinishTime", "raceLastCheckpoint", "raceLastCheckpointTime", "raceBestLapTime" ]))
      {
        local txt = isEmpty ? "" : ::g_mplayer_param_type.getTypeById(hdr).printFunc(item, hdr, table[i])
...
class ::gui_handlers.MPStatistics extends ::gui_handlers.BaseGuiHandlerWT
{
  ...
  defaultRowHeaders         = ["squad", "name", "aircraft", "score", "kills", "groundKills", "navalKills", "aiKills",
                               "aiGroundKills", "aiNavalKills", "assists", "captureZone", "damageZone", "deaths"]
  ...
  function initStats()
  {
    ...
    missionObjectives = ::g_mission_type.getTypeByMissionName(::get_current_mission_name()).objectives
