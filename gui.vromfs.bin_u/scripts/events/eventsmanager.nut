...
class Events
{
  ...
  */
  ...
  function buildCantFlyText(membersData)
  {
    local res = ""
    foreach(member in membersData.members)
    {
      local text = "<color=@userlogColoredText>" + member.name + "</color>" + ::loc("ui/colon")
                   + ::loc(::g_squad_utils.getMemberStatusLocId(member.status))
      res += ((res == "") ? "" : "\n") + text
    }
    return res
  }
  ...
  {
    local msgArr = [::loc("msgbox/squad_not_ready_for_event")]
    local teamTexts = {}
    local lastText = ""
    local identicalTexts = true
    foreach(idx, membersData in cantFlyByTeams)
    {
      local team = ::getTblValue("team", membersData, idx)
      teamTexts[team] <- buildCantFlyText(membersData)
      if (lastText == "")
        lastText = teamTexts[team]
      else if (lastText != teamTexts[team])
        identicalTexts = false
    }

    if (identicalTexts)
      msgArr.append(lastText)
    else
      foreach(team, text in teamTexts)
      {
        msgArr.append(::loc("events/" + team) + ::loc("ui/colon"))
        msgArr.append(text)
      }

    ::showInfoMsgBox(::implode(msgArr, "\n"), "cant_fly")
    ::g_chat.sendMessageToSquadRoom(::implode(msgArr, " "))
  }
  ...
  {
    local teams = getAvailableTeams(event)
