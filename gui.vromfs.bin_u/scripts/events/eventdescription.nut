...
class ::gui_handlers.EventDescription extends ::gui_handlers.BaseGuiHandlerWT
{
  ...
  function getTeamPlayersCountText(team, teamData, roomMGM)
  {
    ...
    local countText = countTbl[team]
    if (countTbl[team] >= ::events.getTeamSize(teamData)
        || countTbl[team] - ::events.getMaxLobbyDisbalance(roomMGM) >= countTbl[otherTeam])
      countText = ::colorize("warningTextColor", countText)

    return ::loc("events/players_count") + ::loc("ui/colon") + countText
  }
