...
class ::gui_handlers.clanPageModal extends ::gui_handlers.BaseGuiHandlerWT
{
  ...
  function fillClanPage()
  {
    ...
    scene.findObject("clan-icon")["background-image"] = "#ui/images/avatars/cardicon_default";

    local regionObj = scene.findObject("clan-region")
    if (!::has_feature("ClanRegions"))
      regionObj.show(false)

    regionObj.setValue(::getFilteredChatMessage(clanData.region, false));
    scene.findObject("clan-motto").setValue(::getFilteredChatMessage(clanData.slogan, false));
    scene.findObject("clan-announcement").setValue(::getFilteredChatMessage(clanData.announcement, false))
    scene.findObject("clan-desc").setValue(getFilteredChatMessage(clanData.desc, false));
  ...
  function drawIcon(nick, presence)
  {
    ...
    imgObj["background-image"] = "#ui/gameuiskin#" + presence.presenceIcon
