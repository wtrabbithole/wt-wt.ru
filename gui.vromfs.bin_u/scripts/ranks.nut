...
//********************************* START: update unit by warpoints.blk *****************************************

    local ac = cost[air.name]
    if (!::u.isDataBlock(ac))
      ac = ::DataBlock() //units list generated by airtags blk, so they can be missing in warpoints.blk

    local unitType = ::get_es_unit_type(air)
    air.cost <- ac.value || 0
    air.costGold <- ac.costGold || 0
    air.repairCost <- ac.repairCost || 0
    ...
    air.freeRepairs <- ac?.freeRepairs ?? freeRepairs
    ...
    air.modifications <- []
    if (::u.isDataBlock(ac.modifications))
...
//********************************* END: update unit by warpoints.blk *****************************************
