...
{
  return ::isInArray(user_opt, ::measure_unit_user_options)
}
...
function find_in_array(arr, val, notFoundValue = -1)
{
...
function find_in_table(tab, val, notFoundValue = null)
{
...
function build_option_blk(text, image, selected, enabled = true, textStyle = "", expImg = false, tooltip = "", addDiv = "")
{
  return ("option {  " +
           ( enabled?     "" : "enable:t='no'; ") +
...
function create_option_switchbox(config)
{
...
function get_bool_option(descr, id, optionIdx)
{
  descr.id = id
...
function get_option(type, context = null)
{
  ...
  switch (type)
  {
    ...
    case ::USEROPT_INSTRUCTOR_GROUND_AVOIDANCE:
      get_bool_option(descr, "instructorGroundAvoidance", ::OPTION_INSTRUCTOR_GROUND_AVOIDANCE); break;
    case ::USEROPT_INSTRUCTOR_GEAR_CONTROL:
      get_bool_option(descr, "instructorGearControl", ::OPTION_INSTRUCTOR_GEAR_CONTROL); break;
    case ::USEROPT_INSTRUCTOR_FLAPS_CONTROL:
      get_bool_option(descr, "instructorFlapsControl", ::OPTION_INSTRUCTOR_FLAPS_CONTROL); break;
    case ::USEROPT_INSTRUCTOR_ENGINE_CONTROL:
      get_bool_option(descr, "instructorEngineControl", ::OPTION_INSTRUCTOR_ENGINE_CONTROL); break;
    case ::USEROPT_INSTRUCTOR_SIMPLE_JOY:
      get_bool_option(descr, "instructorSimpleJoy", ::OPTION_INSTRUCTOR_SIMPLE_JOY); break;
    case ::USEROPT_MAP_ZOOM_BY_LEVEL:
      get_bool_option(descr, "storeMapZoomByLevel", ::OPTION_MAP_ZOOM_BY_LEVEL); break;
...
//      descr.items.append("#options/viewCockpitGunner")
    ...
    case ::USEROPT_INVERTY:
      ...
      descr.controlType = optionControlType.CHECKBOX
      descr.controlName <- "switchbox"
      descr.value = ::get_option_invertY(0) != 0
      break
    ...
    case ::USEROPT_INVERT_THROTTLE:
      descr.id = "invertT"
      descr.items = ["#options/no", "#options/yes"]
      descr.value = ::get_option_invertY(2) == 0 ? 0 : 1
      break
    ...
    case ::USEROPT_GUNNER_INVERTY:
      ...
      descr.controlType = optionControlType.CHECKBOX
      descr.controlName <- "switchbox"
      descr.value = ::get_option_invertY(1) != 0
      break
    ...
    case ::USEROPT_INVERTY_TANK:
      ...
      descr.controlType = optionControlType.CHECKBOX
      descr.controlName <- "switchbox"
      descr.value = ::get_option_invertY(3) != 0
      break
    ...
    case ::USEROPT_CAMERA_SHAKE_MULTIPLIER:
      ...
      descr.value = (::get_option_camera_shake_multiplier() * 50.0).tointeger()
...
//                     "#options/weathershower",
    ...
    case ::USEROPT_USER_SKIN:
      ...
      if (::is_platform_pc && ::has_feature("UserSkins") && ::cur_aircraft_name)
      {
        ...
        local setValue = cdb[::cur_aircraft_name] != null
        ...
        if (skinsBlock)
        {
          for(local i = 0; i < skinsBlock.blockCount(); i++)
          {
            ...
            if (setValue && cdb[::cur_aircraft_name] == table.name)
              descr.value = i + 1
          }
        }
        if (descr.value == null)
...
//                     "#options/dyncount/need_3_wins",   //temporary disabled 3 wins for anniversary event
    ...
    case ::USEROPT_TTV_USER_NAME:
      descr.id = "ttv_user_name"
      descr.controlType = optionControlType.EDITBOX
      descr.controlName <-"editbox"
      descr.value = ::get_ttv_options().ttvUserName == "0"? "" : ::get_ttv_options().ttvUserName
      descr.maxlength <- 25 //twitch max user name
      break
    case ::USEROPT_TTV_PASSWORD:
      descr.id = "ttv_password"
      descr.controlType = optionControlType.EDITBOX
      descr.controlName <-"editbox"
      descr.value = ::get_ttv_options().ttvUserPassword == "0"? "" : ::get_ttv_options().ttvUserPassword
      descr.password <- true
      descr.maxlength <- 25
      break
    case ::USEROPT_TTV_USE_AUDIO:
      descr.id = "ttv_use_audio";
      descr.controlType = optionControlType.CHECKBOX
      descr.controlName <- "switchbox"
      descr.value = ::get_ttv_options().ttvUseAudio
      break;
    case ::USEROPT_TTV_VIDEO_SIZE:
      {
        local opt = ::get_ttv_options();
        local optstr = ""+opt.ttvBroadcastWidth+"x"+opt.ttvBroadcastHeight;
        descr.id = "ttv_video_size";
        descr.items = [];
        descr.values = [];
        descr.value = 0;
        for (local idx = 0; idx < ::ttv_video_sizes.len(); idx++)
        {
          local str = ""+::ttv_video_sizes[idx][0]+"x"+::ttv_video_sizes[idx][1];
          descr.items.append(str);
      ...
      break;
    case ::USEROPT_TTV_VIDEO_BITRATE:
      descr.id = "ttv_video_bitrate"
      local opt = ::get_ttv_options();
      if (opt.ttvBroadcastBitrate)
        descr.value = (opt.ttvBroadcastBitrate.tofloat() / 3500.0 * 100).tointeger()
      else
        descr.value = 30
      break
    ...
    case ::USEROPT_INTERNET_RADIO_ACTIVE:
      ...
      descr.controlType = optionControlType.CHECKBOX
      descr.controlName <- "switchbox"
      descr.value = ::get_internet_radio_options().active
    ...
    case ::USEROPT_INTERNET_RADIO_STATION:
      ...
      descr.value = find_in_array(descr.values, ::get_internet_radio_options().station)
      descr.cb = "update_internet_radio";
      break;
...
function set_option(type, value, descr = null)
{
  ...
  switch (type)
  {
    ...
    case ::USEROPT_INGAME_VIEWTYPE:
      ...
      break
    case ::USEROPT_INVERTY:
      ::set_option_invertY(0, value ? 1 : 0)
      break
    case ::USEROPT_INVERTX:
      ::set_option_invertX(value)
      break
    case ::USEROPT_GUNNER_INVERTY:
      ::set_option_invertY(1, value ? 1 : 0)
      break
    case ::USEROPT_INVERT_THROTTLE:
      ::set_option_invertY(2, value)
      break
    case ::USEROPT_INVERTY_TANK:
      ::set_option_invertY(3, value ? 1 : 0)
      break
    ...
    case ::USEROPT_INVERTCAMERAY:
      ::set_option_camera_invertY(value)
      break
    ...
    case ::USEROPT_CAMERA_SHAKE_MULTIPLIER:
      ::set_option_camera_shake_multiplier(value / 50.0)
      break
    ...
    case ::USEROPT_FONTS_CSS:
      local fontsCss = value ? SCALE_FONTS_CSS : PX_FONTS_CSS
      ::set_gui_option(type, fontsCss)
    ...
    case ::USEROPT_AUTOTRIM:
      ::g_aircraft_helpers.setOptionValue(type,
          descr.values != null ? ::getTblValue(value, descr.values) : value)
      break
    ...
    case ::USEROPT_TTV_USER_NAME:
    case ::USEROPT_TTV_PASSWORD:
      local ttv_options = ::get_ttv_options();
      ttv_options[type == ::USEROPT_TTV_USER_NAME ? "ttvUserName" : "ttvUserPassword"] = value;
      ::set_ttv_options(ttv_options)
      break
    case ::USEROPT_TTV_USE_AUDIO:
      local ttv_options = ::get_ttv_options()
      ttv_options["ttvUseAudio"] = value
      ::set_ttv_options(ttv_options)
      break
    case ::USEROPT_TTV_VIDEO_SIZE:
      if (value in ::ttv_video_sizes)
      {
        local ttv_options = ::get_ttv_options();
        ttv_options.ttvBroadcastWidth = ::ttv_video_sizes[value][0];
        ttv_options.ttvBroadcastHeight = ::ttv_video_sizes[value][1];
        ::set_ttv_options(ttv_options);
      }
      break;
    case ::USEROPT_TTV_VIDEO_BITRATE:
      local ttv_options = ::get_ttv_options();
      ttv_options.ttvBroadcastBitrate = (value.tofloat() / 100.0 * 3500.0).tointeger();
      ::set_ttv_options(ttv_options);
      break
    ...
    case ::USEROPT_FPS_CAMERA_PHYSICS:
      local val = value / 100.0
      ::set_option_multiplier(::OPTION_FPS_CAMERA_PHYS, val)
      break
...
function get_current_clusters_texts()
{
  local clusterOpt = ::get_option(::USEROPT_RANDB_CLUSTER)
  local result = []
  for (local i = 0; i < clusterOpt.values.len(); i++)
...
function show_selected_clusters(textObj)
{
  if (!::checkObj(textObj))
    return
...
function get_unit_preset_img(unitName /*or unit group name*/)
{
  if (!::units_img_preset)
  {
    ::units_img_preset = {}
    local guiBlk = ::configs.GUI.get()
    local blk= guiBlk && guiBlk.units_presets && guiBlk.units_presets[::get_country_flags_preset()]
    if (blk)
