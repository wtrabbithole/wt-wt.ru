...
function append_social_options(options)
{
  ...
  if(!hasTwitch && !::has_feature("Facebook"))
    return null
  ...
  if (hasTwitch)
    block.options.extend([
      [::USEROPT_TTV_USER_NAME, "editbox"],
      [::USEROPT_TTV_PASSWORD, "editbox"],
      [::USEROPT_TTV_USE_AUDIO, "spinner"],
      [::USEROPT_TTV_VIDEO_SIZE, "spinner"],
      [::USEROPT_TTV_VIDEO_BITRATE, "sliderProgress"]]
    )
