class ::mission_rules.SharedPool extends ::mission_rules.Base
{
  ...
  function getExpClassLimitTextByUnit(unit)
  {
    ...
    local limit = ::g_unit_limit_classes.ActiveLimitByUnitRole(
                    ::cut_prefix(expClassName, "exp_", expClassName),
                    activeAtOnce,
  ...
  function getUnitLeftRespawnsByTeamDataBlk(unit, teamDataBlk)
  {
    ...
    if (::u.isDataBlock(limitedClasses))
    {
      ...
      {
        local role = ::cut_prefix(limitedClasses.getParamName(i), "exp_", null)
        if (!role || !::isInArray("type_" + role, unit.tags))
          continue
  ...
  function calcFullUnitLimitsData()
  {
    ...
    if (::u.isDataBlock(limitedClasses))
    {
      ...
      for(local i = 0; i < total; i++)
      {
        local role = ::cut_prefix(limitedClasses.getParamName(i), "exp_", null)
        if (role)
          res.unitLimits.append(::g_unit_limit_classes.LimitByUnitRole(role, limitedClasses.getParamValue(i)))
      }
    }
    ...
    if (::u.isDataBlock(activeLimitsBlk))
    {
      ...
      foreach(expClassName, maxAmount in limitByExpClassName)
        res.unitLimits.append(
          ::g_unit_limit_classes.ActiveLimitByUnitRole(
            ::cut_prefix(expClassName, "exp_", expClassName),
            maxAmount,
