class ::items_classes.Booster extends ::BaseItem
{
  ...
  function getLeftStopSessions()
  {
    ...
    if (spentInSessionTimeMin && ::is_in_flight())
      res -= (::get_usefull_total_time() / TIME_MINUTE_IN_SECONDS_F / spentInSessionTimeMin).tointeger()
    return ::max(0, res)
  }
  ...
  function getExpireFlightTime()
  {
    if (stopProgress == null)
      return -1
    return spentInSessionTimeMin * (getTotalStopSessions() - stopProgress) * TIME_MINUTE_IN_SECONDS
  }
