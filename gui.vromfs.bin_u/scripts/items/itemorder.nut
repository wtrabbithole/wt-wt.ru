  ...
  function getLongDescription()
  {
    ...
    if (timeTotal > 0)
    {
      description += ::loc("items/order/timeTotal") + ::loc("ui/colon")
        + ::colorize("activeTextColor", ::secondsToString(timeTotal, true, true)) + "\n\n"
    }
    ...
    if (delayFromStart > 0)
      ...
      + ::colorize("activeTextColor", ::secondsToString(delayFromStart, true, true)) + "\n"
    ...
    description += ::colorize("grayOptionColor", ::loc("items/order/disabledDifficulties")
      + ::loc("ui/colon") + ::implode(disabledItems, ", "))
