...
class ::gui_handlers.SelectUnit extends ::gui_handlers.BaseGuiHandlerWT
{
  ...
  function initAvailableUnitsArray()
  {
    ...
    unitsList.sort((@(crew, sortUnitsList) function(a, b) {
      local trained = ::getTblValue("trainedSpec", crew, {})
      return sortUnitsList(a, b, trained)
    })(crew, sortUnitsList))

    return needEmptyCrewButton
  }

  function sortUnitsList(a, b, trained)
  {
    if (!a && !b) return 0
    if (!a) return -1
    if (!b) return 1
    if (typeof(a) != "table" && typeof(b) != "table") return 0
    if (typeof(a) != "table") return -1
    if (typeof(b) != "table") return 1
    local trainedA = a.name in trained || !a.trainCost
    local trainedB = b.name in trained || !b.trainCost
    if (trainedA != trainedB)
      return trainedA? 1 : -1
    if (a.rank != b.rank)
      return (a.rank > b.rank)? 1 : -1
    if (::is_default_aircraft(a.name) != ::is_default_aircraft(b.name))
      return ::is_default_aircraft(a.name)? -1 : 1
    if (a.name != b.name)
