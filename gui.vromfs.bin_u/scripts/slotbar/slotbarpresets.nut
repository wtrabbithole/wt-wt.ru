...
class ::gui_handlers.ChooseSlotbarPreset extends ::gui_handlers.BaseGuiHandlerWT
{
  ...
  function reinit(showPreset = null, verbose = false)
  {
    ...
    {
      local title = ::slotbarPresets.getTitle(idx)
      if (idx == activePreset)
  ...
  function updateDescription()
  {
    ...
    if (chosenValue in presets)
    {
      ...
      if (!preset.enabled)
        data += ::format("textarea{ text:t='%s' padding:t='-1@textPaddingBugWorkaround, 8*@sf/@pf' } ",
          ::colorize("badTextColor", ::loc("shop/slotbarPresets/forbidden/unitTypes")))
    ...
    {
      local data = ::format("textarea{ text:t='%s' width:t='pw' } ", ::loc("shop/slotbarPresets/presetUnknown"))
      guiScene.replaceContentFromText(objDesc, data, data.len(), this)
    }
  ...
  function onBtnPresetDelete(obj)
  {
    ...
    local msgText = ::loc("msgbox/genericRequestDelete", { item = ::slotbarPresets.getTitle(chosenValue) })
...
::slotbarPresets <- {
  ...
  function initCountry(countryId)
  {
    ...
    if (selPresetId == null || selPresetId < 0  || selPresetId >= presets[countryId].len())
    {
      selPresetId = null
    ...
    if (selPresetId != null)
    {
      slotbarPreset.title = getTitle(selPresetId, countryId)
      presets[countryId][selPresetId] = slotbarPreset
    }
  ...
  function getDefaultTitle(idx)
  {
    return ::loc("shop/slotbarPresets/item", {number = idx + 1})
  }

  function getTitle(idx, countryId = null)
  {
    if (!countryId)
      countryId = ::get_profile_info().country
    local name = getDefaultTitle(idx)
    if (countryId in presets && idx in presets[countryId])
      return ::getTblValue("title", presets[countryId][idx], name)
    ...
    if (presets[countryId].len() > 0)
    {
      ...
      foreach (idx, p in presets[countryId])
      {
        list.append(::g_string.join([p.selected,
                               ...
                               ::getTblValue("title", p, getTitle(idx)),
  ...
  function canLoadPreset(preset, isSilent = false)
  {
    ...
    if (!isSilent)
      ::showInfoMsgBox(::loc("msg/cantUseUnitInCurrentBattle",
                       { unitName = ::colorize("userlogColoredText", getPresetTitle(preset)) }))
    return false
  }
  ...
  function getPresetsList(countryId)
  {
    ...
    if (blk)
    {
      ...
      foreach (idx, strPreset in presetsBlk)
      {
        ...
        preset.selected = data[0]
        local title = validatePresetName(::getTblValue(3, data, getTitle(idx)))
        if (title.len())
          preset.title = title
        preset.gameModeId = ::getTblValue(4, data, null)
        ...
        preset.selected = preset.selected.tointeger()
        _updateInfo(preset)
        list.append(preset)
  ...
  function _createPresetTemplate(presetIdx)
  {
    return {
      ...
      crews = []
      selected = -1
      title = getDefaultTitle(presetIdx)
      gameModeId = ""
  ...
  function updatePresetFromSlotbar(preset, countryId)
  {
    ...
    preset.gameModeId = ::game_mode_manager.getCurrentGameModeId()
    _updateInfo(preset)
    return preset
  }
  ...
  function getPresetTitle(preset, presetIdx = 0) //better to push this info right inside preset, but a lot of places where need to fix it
  {
    return ::getTblValue("title", preset) || getTitle(presetIdx)
  }
}
