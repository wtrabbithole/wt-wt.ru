...
function build_aircraft_item(id, air, params = {})
{
  ...
  else if (air && ::isUnitGroup(air)) //group of aircrafts
  {
    ...
    foreach(a in air.airsGroup)
    {
      ...
      if (isInResearch || ::canResearchUnit(a))
      {
        researchingUnit = a
...
function get_unit_rank_text(unit, crew = null, showBR = false, ediff = -1)
{
  ...
  if (::is_unit_modern(unit))
  {
    local rankInfo = ::get_modern_unit_fake_rank_info(unit)
    return showBR ? ::format("%.1f", rankInfo.br) : ::get_roman_numeral(rankInfo.rank)
  }
...
*/
  ...
  if (handler && ("curSlotCountryId" in handler))
  {
    ...
    if (fullSlotbar && ::SessionLobby.canChangeCrewUnits())
