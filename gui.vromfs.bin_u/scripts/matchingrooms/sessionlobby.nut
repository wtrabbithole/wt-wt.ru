...
function SessionLobby::setSettings(_settings, notify = false, checkEqual = true)
{
  ...
  roomUpdated = notify || !isRoomOwner || !isInRoom()
  if (!roomUpdated)
...
function SessionLobby::getMemberInfo(idx)
{
  if (!(idx in members))
    return null

  local m = members[idx]
  local pub = ("public" in m)? m.public : {}
  local res = {
    memberId = m.memberId
    userId = m.userId
    name = m.name
    isLocal = m.userId == ::my_user_id_str
    spectator = ::getTblValue("spectator", m, false)
    isBot = false
  }
  ...
  if (hasSessionInLobby())
  {
    if (res.state == ::PLAYER_IN_LOBBY_NOT_READY || res.state == ::PLAYER_IN_LOBBY_READY)
      res.state = isMemberInSession(m) ? ::PLAYER_IN_LOBBY_READY : ::PLAYER_IN_LOBBY_NOT_READY
  }
...
function SessionLobby::getMemberByName(userName)
{
  if (userName == "")
    return null
  foreach (key, member in members)
    if (member.name == userName)
      return member
  return null
}

function SessionLobby::getMembersInfoList()
{
  local res = []
  foreach(idx, member in members)
    res.append(getMemberInfo(idx))
  return res
}
...
function SessionLobby::getMembersCount()
{
  local res = 0
  foreach(m in members)
    if (!isMemberHost(m))
...
function SessionLobby::getMaxMembersCount()
{
  return ("players" in settings)? settings.players : 0
}
...
function SessionLobby::isUnitAllowed(unit)
{
  ...
  local teamsToCheck = team == Team.Any
    ? [Team.A, Team.B]
    : [team]
  foreach(team in teamsToCheck)
  {
    ...
    hasTeamData = true
    if (::events.isUnitAllowedByTeamData(teamData, unit.name, getCurRoomEdiff()))
      return true
  }
  return !hasTeamData
}
