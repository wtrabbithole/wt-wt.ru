...
class ::MenuChatHandler extends ::gui_handlers.BaseGuiHandlerWT
{
  ...
  function switchCurRoom(roomId, needUpdateWindow = true)
  {
    if (roomId == curRoom)
      return
    curRoom = roomId
  ...
  function onRoomChanged()
  {
    ...
    if (roomData.id == curRoom && !sceneChanged)
      return false
    ...
    if (!::g_chat.isThreadsView)
      showSearch(::g_chat.isSystemChatRoom(curRoom))
  ...
  function updateRoomTabByIdx(idx, room, listObj = null)
  {
    ...
    roomTab.findObject("room_txt_"+idx).setValue(room.getRoomName())
  }
  ...
  function updateUsersList()
  {
    ...
    guiScene.setUpdatesEnabled(false, false)
    local roomData = ::g_chat.getRoomById(curRoom)
    local listObj = scene.findObject("users_list")
    local leftObj = scene.findObject("middleLine")
    if (!roomData || !roomData.havePlayersList || (!showPlayersList && !alwaysShowPlayersList()))
    {
      leftObj.show(false)
  ...
  {
    if (!checkScene() || !showPlayersList)
      return

    local readyShow = curRoom == ::g_chat.getMySquadRoomId() && ::g_squad_manager.canSwitchReadyness()
  ...
  {
    local squadRankTextObj = showSceneBtn("squad_rank_text", curRoom == ::g_chat.getMySquadRoomId())
  ...
  function updateUserPresence(listObj, idx, contact)
  {
    ...
    if (obj)
    {
      ...
      local img = (inMySquad && contact.presenceIcon)? "#ui/gameuiskin#" + contact.presenceIcon : ""
      local img2 = ""
      local voiceIcon = ""
      if (inMySquad)
      {
  ...
  function updatePresenceContact(contact)
  {
    ...
    foreach(idx, user in roomData.users)
      if (user.name == contact.name)
      {
        ...
        local listObj = scene.findObject("users_list")
        updateUserPresence(listObj, idx, contact)
        if (curRoom == ::g_chat.getMySquadRoomId())
          updateSquadInfo()
        return
      }
  }
...
//    if (event == ::GCHAT_EVENT_TASK_RESPONSE || event == ::GCHAT_EVENT_TASK_ERROR)
  ...
  {//messageAuthor can be as string - Player nick, and as table - player contact.
   //after getting type, and acting accordingly, name must be string and mean name of player
    ...
    local userColor = ""
    if (privateMsg)
      userColor = senderPrivateColor
    else
      userColor = myself ? senderMeColor : senderColor
    ...
    foreach(roomData in ::g_chat.rooms)
    {
      if ((roomId=="") || roomData.id == roomId)
      {
        ...
        if (roomData.id == curRoom || roomData.hidden)
        {
          updateChatText()
        }

        if ((roomId!="" && (!::last_chat_scene_show || roomData.id != curRoom)) &&
           ((privateMsg && !myPrivate) || important))
        {
          ...
          showRoomPopup(fullName, msg, roomData.id)
                 ...
                 && (!::last_chat_scene_show || !::g_chat.isSystemChatRoom(curRoom))
  ...
  function colorMyNameInText(msg)
  {
    ...
    while (counter+::my_user_name.len() <= msg.len())
    {
          ...
          ::isInArray(msg.slice(nameEndPos, nameEndPos+1),     ::punctuation_list))
      {
        ...
        local msgProcessedPart = msgStart +"<color=@mainPlayerColor>"+msgName+"</color>";
  ...
  function checkNewMessages()
  {
    ...
    local room = ::g_chat.getRoomById(curRoom)
    if (!room)
      return

    room.newImportantMessagesCount = 0
  ...
  function onMessage(db)
  {
    ...
    else if (db.type == "groupchat" || db.type == "chat")
    {
      ...
      if (!db.sender.service)
      {
        ...
        if (::g_chat.isSystemUserName(user))
        {
          ...
          if (db.message.len() >= nameLen && db.message.slice(0, nameLen) == ::my_user_name)
            ::sync_handler_simulate_signal("profile_reload")
        }
      }
      addRoomMsg(roomId, userContact || user, ::g_chat.clampMsg(db.message),
    ...
    else if (db.type == "error")
    {
      ...
      checkLastActionRoom()
      if (db.error.errorName == chatErrorName.NO_SUCH_NICK_CHANNEL)
      {
        local userName = db.error.param1
      ...
      else if (db.error.errorName == chatErrorName.CANNOT_JOIN_THE_CHANNEL && db.error.param1.len() > 1)
      {
        local roomName = db.error.param1
        local wasPasswordEntered = ::getTblValue(roomName, roomJoinParamsTable, "") != ""
        local locId = wasPasswordEntered ? "chat/wrongPassword" : "chat/enterPassword"
        local params = {
          title = roomName.slice(1)
          editboxHeaderText = ::format(::loc(locId), roomName.slice(1))
          allowEmpty = false
          okFunc = ::Callback((@(roomName) function(pass) {
            joinRoom(roomName, pass)
          })(roomName), ::menu_chat_handler)
        }

        ::gui_modal_editbox_wnd(params)
      }
      if (db.error.param1 && db.error.param1 == ::g_chat.getMySquadRoomId() &&
          ::isInArray(db.error.errorName, [chatErrorName.NO_SUCH_CHANNEL, chatErrorName.NO_SUCH_NICK_CHANNEL]))
      {
        leaveSquadRoom()
      }
      else {
        ...
        if (senderFrom && roomRegexp.match(senderFrom) && ::g_chat.getRoomById(senderFrom))
          roomId = senderFrom
        ...
        local errMsg = ::loc("chat/error/" + db.error.errorName, locParams)
        addRoomMsg(roomId, "", errMsg)
        if (roomType.isErrorPopupAllowed)
          showRoomPopup(null, errMsg, roomId)
        if (db.error.param0 && roomId!=db.error.param1)
          addRoomMsg(db.error.param1, "", errMsg)
      }
    }
    else
  ...
  function onJoinRoom(event, db, taskConfig)
  {
    if (event != ::GCHAT_EVENT_TASK_ERROR && db && db.type!="error")
    {
      ...
      if (needNewRoom)
      {
        addRoom(taskConfig.roomId, taskConfig.customScene, taskConfig.ownerHandler)
        updateRoomsList()
      }
  ...
  function addRoom(id, customScene = null, ownerHandler = null, selectRoom = false)
  {
    ...
    local r = {
      id = id
      type = roomType
      getRoomName = function() { return type.getRoomName(id) }
  ...
  function onUsersListActivate(obj)
  {
    ...
    local roomData = ::g_chat.getRoomById(curRoom)
    if (!roomData || roomData.users.len() <= value || !checkScene())
      return

    ::g_chat.showPlayerRClickMenu(roomData.users[value].name, curRoom, null, obj.getChild(value).getPosRC())
  }
      ...
      foreach(cmd in ::available_cmd_list)
            ...
            || (msg.len() == (cmd.len()+1) && msg.slice(1, cmd.len()+1) == cmd))
        {
          ...
          if (cmd == "help" || cmd == "shelp")
            addRoomMsg(curRoom, "", ::loc("menuchat/" + cmd))
          else if (cmd == "edit")
            ::g_chat.openModifyThreadWndByRoomId(curRoom)
          else if (cmd == "msg")
          ...
          else if (cmd == "p_check")
          {
            if (!hasParam)
            {
              addRoomMsg(curRoom, "", ::loc("chat/presenceCheckArg"));
              return null;
            }

            if ( !::is_myself_anyof_moderators() )
            {
              addRoomMsg(curRoom, "", ::loc("chat/presenceCheckDenied"));
              return null;
            }
          ...
          else if (cmd == "join" || cmd == "part")
          {
            if (cmd == "join")
            {
              if (!hasParam)
              {
                addRoomMsg(curRoom, "", ::loc("chat/error/461"));
                return null;
              }
          ...
          else if (cmd == "invite")
          {
            if (curRoom!="")
            {
              if (curRoom==::g_chat.getMySquadRoomId())
              {
                ...
                ::gchat_raw_command(msg.slice(1) + " " + curRoom)
            } else
              addRoomMsg(curRoom, "", ::loc(::g_chat.CHAT_ERROR_NO_CHANNEL))
          }
  ...
  function gchatRawCmdWithCurRoom(msg, cmd)
  {
    ...
    if (::g_chat.isSystemChatRoom(curRoom))
      addRoomMsg(curRoom, "", ::loc("chat/cantWriteInSystem"))
    else
    {
      if (msg.len() > cmd.len()+2)
        ::gchat_raw_command(msg.slice(1, cmd.len()+2) + curRoom + " " + msg.slice(cmd.len()+2))
      else
        ::gchat_raw_command(msg.slice(1) + " " + curRoom)
    }
  }
  ...
  function squadMsg(msg)
  {
    ...
    if (curRoom!=sRoom) addRoomMsg(curRoom, "", msg)
  }
  ...
  function inviteToSquadRoom(playerName, delayed=false)
  {
    ...
    {
      addRoomMsg(curRoom, "", ::loc("msgbox/notAvailbleYet"))
      return false
    }
    ...
    {
      addRoomMsg(curRoom, "", ::loc("squad/only_leader_can_invite"))
      return false
    }
  ...
  function checkValidAndSpamMessage(msg, room = null, isPrivate = false)
  {
    ...
    if (::is_chat_message_allowed(msg))
      return true
    addRoomMsg(room? room : curRoom, "", ::loc("charServer/ban/reason/SPAM"))

    return false
  }

  function checkAndPrintDevoiceMsg(room = null)
  {
    if (!room)
      room = curRoom
  ...
  function getPrivateData(msg, room = null)
  {
    if (msg.slice(0, 1)=="\\" || msg.slice(0, 1)=="/")
    {
      msg = msg.slice(1)
    ...
    if (!room)
      room = curRoom
    if (room != "" && room.slice(0, 1)!="#")
    {
      return { user = room, msg = msg };
    }
    return null
  }
  ...
  function onChatPrivate(data)
  {
    ...
    if (gchat_chat_private_message(curRoom/*+ roomHost*/, data.user, data.msg))
    {
      if (needLocalEcho)
        addRoomMsg(curRoom, data.user, data.msg, true, true)

      local blocked = ::isPlayerNickInContacts(data.user, ::EPL_BLOCKLIST)
      if (blocked)
        addRoomMsg(curRoom, "", format(::loc("chat/cantChatWithBlocked"), "<Link="+::g_chat.generatePlayerLink(data.user)+">"+data.user+"</Link>"))
      else
        if (data.user!=curRoom)
        {
...
//    else
//      addRoomMsg(curRoom, "", format(::loc("chat/userNotFound"), data.user)) - it will be usual error

//    if (task>=0)
//      chatTasks.append({ task = task, handler = onPrivateSend, roomId = curRoom, userTo = data.user, msg = data.msg })
//    dlog("GP: task = " + task)

//      gchat_send_message(jid, data.msg)
  }
/*
  function onPrivateSend(event, db, taskConfig)
  {
    if (event == ::GCHAT_EVENT_TASK_ERROR || !db || db.type=="error")
      addRoomMsg(taskConfig.roomId, "", format(::loc("chat/userNotFound"), taskConfig.userTo))
    else
      addRoomMsg(taskConfig.roomId, taskConfig.userTo, taskConfig.msg, true, true)

    //    addRoomMsg(curRoom, "", format(::loc("chat/userNotFound"), data.user))
  }
*/
  ...
  function onUserList(obj, lclick)
  {
    ...
    local name = obj.text
    local roomData = ::g_chat.getRoomById(curRoom)
    if (roomData && checkScene())
      if(roomData.users.len() > num)
        {
          name = roomData.users[num].name
  ...
  function joinCustomObjRoom(sceneObj, roomId, password, ownerHandler)
  {
    ...
    if (roomData)
    {
      roomData.customScene = sceneObj
      roomData.ownerHandler = ownerHandler
      roomData.joined = true
      afterReconnectCustomRoom(roomId)
      updateChatText()
    }

    joinRoom(roomId, password, (@(roomId) function() {
        afterReconnectCustomRoom(roomId)
      })(roomId), sceneObj, ownerHandler)
  }
  ...
  senderColor = "@chatSenderColor"
  senderMeColor = "@chatSenderMeColor"
  senderPrivateColor = "@chatSenderPrivateColor"
...
{
  if (::menu_chat_handler)
    ::menu_chat_handler.openInviteMenu.call(::menu_chat_handler, menu)
}
