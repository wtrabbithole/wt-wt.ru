...
function g_chat::joinSquadRoom(callback)
{
  ...
  if (::menu_chat_handler)
    ::menu_chat_handler.joinRoom.call(::menu_chat_handler, name, password,
                                      (@(callback) function () {
                                        if (callback != null)
                                          callback()
                                      })(callback)
...
function g_chat::getPlayerRClickMenu(playerName, roomId = null, contact = null)
{
  if (!contact)
...
{
  local menu = getPlayerRClickMenu(playerName, roomId, contact)
...
function g_chat::sendMessageToSquadRoom(message)
{
  ...
  if (::checkObj(::menu_chat_handler))
    ::menu_chat_handler.sendMessageToRoom(message, squadRoomId, true)
  else
    ::gchat_chat_message(squadRoomId, message)
}
