...
function logout_with_msgbox(params)
{
  ...
  local reason = params.reason & 0xFFFFFFFF
  ::g_matching_connect.logoutWithMsgBox(reason, message, params.reasonDomain)
}
...
function exit_with_msgbox(params)
{
  local message = "message" in params ? params["message"] : null
  local reason = params.reason & 0xFFFFFFFF
  ::g_matching_connect.exitWithMsgBox(reason, message, params.reasonDomain)
}
