...
function checkBrokenAirsAndDo(repairInfo, handler, startFunc, canRepairWholeCountry = true, cancelFunc = null)
{
  ...
  local repairAll = (@(handler, repairInfo, startFunc, canRepairWholeCountry) function() {
      ::repairAllAirsAndApply(handler, repairInfo.broken_countries, startFunc, canRepairWholeCountry, repairInfo.repairCost)
    })(handler, repairInfo, startFunc, canRepairWholeCountry)

  local onCancel = (@(handler, cancelFunc) function() { ::call_for_handler(handler, cancelFunc) })(handler, cancelFunc)
  ...
  else if (repairInfo.broken_countries.len() > 0)
  {
    ...
    handler.msgBox("no_aircrafts", msgText,
       [
         ["ContinueWithoutRepair", (@(startFunc, handler) function() { startFunc.call(handler) })(startFunc, handler)],
         ["RepairAll", repairAll],
         ["cancel", onCancel]
       ], "RepairAll")
    return
...
function repairAllAirsAndApply(handler, broken_countries, afterDoneFunc, canRepairWholeCountry = true, totalRCost=0)
{
  if (!handler)
    return
  ...
  if (totalRCost && !::old_check_balance_msgBox(totalRCost))
    return
  ...
  if (taskId >= 0)
  {
    ...
    ::add_bg_task_cb(taskId, (@(handler, progressBox, broken_countries, afterDoneFunc, canRepairWholeCountry) function() {
      ::destroyMsgBox(progressBox)
      ::repairAllAirsAndApply(handler, broken_countries, afterDoneFunc, canRepairWholeCountry)
    })(handler, progressBox, broken_countries, afterDoneFunc, canRepairWholeCountry))
  }
}
