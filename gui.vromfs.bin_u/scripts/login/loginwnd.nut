class ::gui_handlers.LoginWndHandler extends ::BaseGuiHandler
{
  ...
  {
    return ::steam_is_running() && !::load_local_custom_settings("usedSteamLinkAccountOnce", false)
  }
  ...
  {
    local loggedInOnce = ::load_local_custom_settings("loggedInOnce", false)
    if (loggedInOnce != true)
    {
      ::save_local_custom_settings("loggedInOnce", true)
      ::skip_steam_confirmations = true
    }

    if (::steam_is_running())
      ::save_local_custom_settings("showNewSteamLogin", true)
      ...
      case ::YU2_OK:
        if (isTryLinkSteamAccount())
        {
          ...
          if (res == ::YU2_OK || res == ::YU2_ALREADY)
            ::save_local_custom_settings("usedSteamLinkAccountOnce", true)
        }
