...
function is_unit_modern(unit)
{
  return ::isInArray("modern", ::getTblValue("tags", unit, []))
}
...
function showAirInfo(air, show, holderObj = null, handler = null, params = null, canChangeAir = true)
{
  ...
  local rankVisual = air.rank
  local battleRatingVisual = battleRating
  if (::is_unit_modern(air))
  {
    local rankInfo = ::get_modern_unit_fake_rank_info(air)
    battleRatingVisual = rankInfo.br
    rankVisual = rankInfo.rank
  }
  ...
  if (::checkObj(ageObj))
  {
    ...
    if (::checkObj(yearsObj))
      yearsObj.setValue(::getUnitRankName(rankVisual))
  }

  holderObj.findObject("aircraft-battle_rating-header").setValue(::loc("shop/battle_rating") + ::loc("ui/colon"))
  holderObj.findObject("aircraft-battle_rating").setValue(format("%.1f", battleRatingVisual))
...
//  holderObj.findObject("aircraft-range").setValue(::countMeasure(2, air.shop.range * 1000.0))
  ...
  local unitBlk = ::DataBlock(::get_unit_file_name(air.name))
  local modernUnitParamsBlk = ::is_unit_modern(air) && ::u.isDataBlock(unitBlk.ui) ? unitBlk.ui : null

  local isTank = ::isTank(air)
  local isModernTank = isTank && modernUnitParamsBlk != null

  local modernTankRows = [
    "aircraft-armorThicknessEquivalent-tr"
    "aircraft-armorThicknessCumulativeHull-tr"
    "aircraft-armorThicknessCumulativeTurret-tr"
    "aircraft-armorPiercing-separator"
  ]
  foreach (id in modernTankRows)
    ::showBtn(id, isModernTank, holderObj)

  if (isTank)
  {
    local labelCfg = {
      armorThicknessHull   = isModernTank ?
        ::loc("shop/armorThicknessEquivalent/kinetic/hull") :
        ::loc("shop/armorThicknessHull")
      armorThicknessTurret = isModernTank ?
        ::loc("shop/armorThicknessEquivalent/kinetic/turret") :
        ::loc("shop/armorThicknessTurret")
    }
    foreach (id, val in labelCfg)
    {
      local labelObj = holderObj.findObject("aircraft-" + id + "-title")
      if (::check_obj(labelObj))
        labelObj.setValue(val)
    }
  }

  if (modernUnitParamsBlk)
  {
    if (::is_helicopter(air))
    {
      local rows = {
        mass          = ::format("%.1f %s", ((modernUnitParamsBlk.mass || 0.0) / 1000.0), ::loc("measureUnits/ton"))
        horsePowers   = (modernUnitParamsBlk.engines || 0) + " x " + ::g_measure_type.HORSEPOWERS.getMeasureUnitsText(modernUnitParamsBlk.engineHorsePowers || 0.0)
        maxSpeed      = ::g_measure_type.SPEED.getMeasureUnitsText(modernUnitParamsBlk.maxMovingSpeed || 0.0)
        climbSpeed    = ::countMeasure(3, modernUnitParamsBlk.climbSpeed || 0.0)
        altitude      = ::countMeasure(1, modernUnitParamsBlk.maxAlt || 0.0)
        weaponPresets = ""
        speedAlt      = ""
        turnTime      = ""
        airfieldLen   = ""
    ...
    {
      local currentParams = {}
      foreach (key, val in modernUnitParamsBlk)
        if (::u.isPoint3(val))
        currentParams[key] <- [ val.x, val.y, val.z ]

      local mmLoc = ::loc("measureUnits/mm")
      local mLoc  = ::loc("measureUnits/meters_alt")

      local valuesCfg = {
        armorThicknessHull              = mmLoc
        armorThicknessCumulativeHull    = mmLoc
        armorThicknessTurret            = mmLoc
        armorThicknessCumulativeTurret  = mmLoc
        armorPiercing                   = mmLoc
        armorPiercingDist               = mLoc
      }

      foreach (id, unitsLoc in valuesCfg)
      {
        local paramsArray = ::getTblValue(id, currentParams, [])
        local valObj = holderObj.findObject("aircraft-" + id)
        if (!paramsArray.len() || !::check_obj(valObj))
          continue

        local val = ::implode(::u.map(paramsArray, function (v) { return ::format("%d", v) }), " / ") + " " + unitsLoc
          valObj.setValue(val)
      }
    }
  }
...
//          holderObj.findObject("aircraft-full_repair_time-tr").show(false)
  ...
  if (::checkObj(obj))
  {
    ...
    {
      local rankInfo = ::get_modern_unit_fake_rank_info(air)
      minAge = rankInfo.levelingAgeMin
      maxAge = rankInfo.levelingAgeMax
    }
...
function get_modern_unit_fake_rank_info(unit)
{
  local cacheKey = "fakeRankInfo"
  local info = ::getTblValue(cacheKey, unit)
  if (info)
    return info
  ...
  local rank = ::getTblValueByPath("ui.rank", unitBlk) ||
    ::getTblValue(unit.name, modernUnitRanks) ||
    unit.rank

  res.rank = rank
  res.br = ::calc_battle_rating_from_rank((1 + rank) * 4)
  res.levelingAgeMin = ::max(1, rank - 1)
  res.levelingAgeMax = ::min(rankMax, rank + 1)
