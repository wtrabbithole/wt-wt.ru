...
//    optionsContainers.push(container.descr)
  ...
  function onMedalTooltipOpen(obj)
  {
    ...
    local completed = config.maxVal <= 0 || config.curVal >= config.maxVal
    ::build_unlock_desc(config, {showProgress = !completed, showCost = !completed})
    ...
    local showProgressBar = config.type!="char_always_progress"
    if (!completed)
    {
      local progressPart = config.curVal / config.maxVal.tofloat() * 1000;
      local pObj = obj.findObject("progress")
      pObj.setValue(progressPart)
      pObj.show(showProgressBar)
    }
    else if(config.text != "")
  ...
  function fillUnlockInfo(unlockBlk, unlockObj)
  {
    ...
    ::g_unlock_view.fillUnlockFav(itemData, unlockObj)
  }
