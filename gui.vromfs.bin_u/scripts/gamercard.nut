function fill_gamer_card(cfg = null, show = true, prefix = "gc_", scene = null, save_scene=true)
{
    ...
    if (::checkObj(obj))
      switch(name)
      {
        ...
        case "gold":
          ...
          break
        case "balance":
          local tooltipText = ::getWpPriceText(::colorize("activeTextColor", val), true) + "\n" + ::loc("mainmenu/warpoints")
          ...
          obj.setValue(::getShortTextFromNum(val))
          break
        case "free_exp":
          local tooltipText = ::getFreeRpPriceText(::colorize("activeTextColor", val), true) + "\n" + ::loc("currency/freeResearchPoints/desc")
          ...
          if (::checkObj(textObj))
            textObj.setValue(::getShortTextFromNum(val))
          break
        case "name":
          if (::u.isEmpty(val))
  ...
  local buttonsEnableTable = {
                                ...
                                gc_contacts = canHaveFriends && ::ps4_is_ugc_enabled() && ::ps4_is_chat_enabled() && disableForPs4Temporary
                                gc_chat_btn = canChat && ::ps4_is_ugc_enabled() && ::ps4_is_chat_enabled() && disableForPs4Temporary
  ...
  local allowDecreaseFont = "no"
  if (::is_low_width_screen())
  {
    allowDecreaseFont = "yes"
    local shopObj = getObj("gc_shop_txt")
    if (::checkObj(shopObj))
    {
      shopObj.setValue("")
      getObj("shoptext_wink").setValue("")
    }
  }

  local battleButtonObj = getObj("gamercard_tobattle")
  if (::checkObj(battleButtonObj))
    battleButtonObj.allowDecreaseFont = allowDecreaseFont

  ::update_discount_notifications(scene)
  ::setVersionText(scene)
  ::server_message_update_scene(scene)

  if (::g_login.isLoggedIn() && scene != null)
  {
    ::set_inventory_new_items_icon_visible(::ItemsManager.getNumUnseenItems(true) > 0, scene)
    ::set_shop_new_items_icon_visible(::ItemsManager.getNumUnseenItems(false) > 0, scene)
    ::set_inventory_icon_visible(::ItemsManager.isEnabled(), scene)
  }
...
function update_gamercards()
{
  ...
  ::set_inventory_new_items_icon_visible(::ItemsManager.getNumUnseenItems(true) > 0)
  ::set_shop_new_items_icon_visible(::ItemsManager.getNumUnseenItems(false) > 0)
  ::set_inventory_icon_visible(::ItemsManager.isEnabled())
}
