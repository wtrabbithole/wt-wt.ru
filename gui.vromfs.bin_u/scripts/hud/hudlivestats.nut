...
::g_hud_live_stats <- {
  ...
  columnsOrder = {
    [::GT_VERSUS] = {
      [LIVE_STATS_MODE.SPAWN] = [ "captureZone", "damageZone", "kills", "groundKills", "navalKills", "assists",
                                  "aiKills", "aiGroundKills", "aiNavalKills", "score" ],
      [LIVE_STATS_MODE.FINAL] = [ "captureZone", "damageZone", "kills", "groundKills", "navalKills", "assists",
                                  "aiKills", "aiGroundKills", "aiNavalKills", "deaths", "score" ],
      [LIVE_STATS_MODE.WATCH] = [ "name", "score", "kills", "groundKills", "navalKills", "assists",
                                  "captureZone", "damageZone", "aiKills", "aiGroundKills", "aiNavalKills", "deaths" ],
    },
    [::GT_RACE] = {
...
function g_hud_live_stats::getState(playerId = null, diffState = null)
{
  ...
  local state = {
    player    = isHero ? ::get_local_mplayer() : ::get_mplayer_by_id(playerId)
...
function g_hud_live_stats::update(obj = null, dt = 0.0)
{
  ...
  foreach (id in curColumnsOrder)
  {
    local visValue = visState && ::getTblValue(id, visState.player, "")
    local value = ::getTblValue(id, state.player, "")
    local param = ::g_mplayer_param_type.getTypeById(id)

    if (visValue == value && !param.isVirtual)
      continue

    local isValid = value != "" && (value != param.defVal || param.isVirtual)
    local text = isValid ? param.printFunc(value, id, state.player) : ""
    local show = text != ""
