...
class ::CrewTakeUnitProcess
{
  ...
  stepsList = {
    ...
    [CTU_PROGRESS.CHECK_AVAILABILITY] = function()
    {
      ...
      local needCheckAllowed = !::SessionLobby.isInvalidCrewsAllowed()
        && (!unit || !::SessionLobby.isUnitAllowed(unit) || ::isUnitBroken(unit))

      if (needCheckAllowed || (prevUnit && !unit))
      {
        local hasOtherUnits = false
        ...
        if (crewsData)
          foreach(c in crewsData.crews)
          {
            ...
            if (!cUnit
                || (needCheckAllowed && (!::SessionLobby.isUnitAllowed(cUnit) || ::isUnitBroken(cUnit))))
              continue
