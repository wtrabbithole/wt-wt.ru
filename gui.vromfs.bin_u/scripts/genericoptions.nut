...
class ::gui_handlers.GroupOptionsModal extends ::gui_handlers.GenericOptionsModal
{
  ...
  function fillSocialOptions(group)
  {
    ...
    local hasTwitch = ::has_feature("Twitch")
    local tObj = showSceneBtn("twich_frame", hasTwitch)
    if(hasTwitch && tObj)
      fillTwitchOptions(group)
  }
  ...
  function fillTwitchOptions(group)
  {
    local config = optGroups[group]

    if ("options" in config)
      fillOptionsList(group, "twitchOptions")

    fillShortcutInfo("ID_TOGGLE_TWITCH_STREAMING", "ttv_shortcut");
  }

  function onAssignTwitchButton()
  {
    assignButtonWindow(this, bindTwitchButton);
  }

  function bindTwitchButton(devs, btns)
  {
    bindShortcutButton(devs, btns, "ID_TOGGLE_TWITCH_STREAMING", "ttv_shortcut");
  }

  function onClearTwitchButton()
  {
    onClearShortcutButton("ID_TOGGLE_TWITCH_STREAMING", "ttv_shortcut");
  }
