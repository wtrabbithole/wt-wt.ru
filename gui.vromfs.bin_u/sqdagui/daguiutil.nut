...
function g_dagui_utils::adjustWindowSize(wndObj, listObj, sizeX, sizeY, spaceX, spaceY)
{
  ...
  adjustWindowSizeByConfig(wndObj, listObj, res)
  return res
}

//config generated by countSizeInItems
function g_dagui_utils::adjustWindowSizeByConfig(wndObj, listObj, config)
{
  ...
  local wndSizeX = ::min(wndSize[0], wndSize[0] - listSize[0] + (config.spaceX + config.itemsCountX * (config.sizeX + config.spaceX)))
  local wndSizeY = ::min(wndSize[1], wndSize[1] - listSize[1] + (config.spaceY + config.itemsCountY * (config.sizeY + config.spaceY)))
