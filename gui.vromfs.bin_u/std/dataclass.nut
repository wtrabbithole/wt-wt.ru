...
local function mkClassFields(fields){
  return fields.map(@(v) _cfield(v[0], v[1])).reduce(addNewline1)
}
local function mkPosFieldInit(fieldname, def){
    return "this.{0} = {0}".subst(fieldname)
}
...
local function mkTableFieldInit(fieldname, firstarg, def){
  def = (def != null)
    ? " ?? {0}".subst(def)
    : ""
  return "this.{0} = {1}?.{0}{2}".subst(fieldname, firstarg, def)
}
local function mkArg(name, def){
  return "{0} = {1}".subst(name, def)
}
...
local function Dataclass(fields, params = defParams){
  ...
  local args = fields.map(@(v) mkArg(v[0], v[1])).reduce(addComma)
